...
export function getPeriodDateRange(period: PeriodFilter, customFrom?: string, customTo?: string): DateRange {
  const now = new Date();
...
export function getTrailing12Months(): Array<{ year: number; month: number; key: string; label: string }> {
  const result = [];
  const now = new Date();
