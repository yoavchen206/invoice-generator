import { createAuthenticatedClient } from '../lib/invoice4uClient';
import type { Invoice, Client, CreateInvoiceRequest, CreateClientRequest, UpdateClientRequest } from '@yoavchu/shared';
...
export class Invoice4uService {
  ...
  private getAxiosClient() {
    return createAuthenticatedClient(this.token);
  }
  ...
  async createInvoice(payload: CreateInvoiceRequest): Promise<Invoice> {
    if (this.useMock) {
      ...
      if (payload.clientId) {
        ...
        if (!found) throw { status: 404, code: 'NOT_FOUND', message: 'Client not found.' };
        client = found;
  ...
  async getClients(params?: { search?: string; page?: number; limit?: number }): Promise<{ clients: Client[]; total: number }> {
    if (this.useMock) {
  ...
  async createClient(payload: CreateClientRequest): Promise<Client> {
    if (this.useMock) {
  ...
  async updateClient(clientId: string, payload: UpdateClientRequest): Promise<Client> {
    if (this.useMock) {
      const idx = mockClients.findIndex(c => c.id === clientId);
      if (idx === -1) throw { status: 404, code: 'NOT_FOUND', message: 'Client not found.' };
  ...
  static async login(email: string, password: string): Promise<{ token: string; userId: string }> {
    ...
    const { invoice4uClient } = await import('../lib/invoice4uClient');
  ...
  private normalizeInvoice(data: Record<string, unknown>): Invoice {
    ...
    return {
      ...
      client: this.normalizeClient((data.client || data.Client || {}) as Record<string, unknown>),
      lineItems: ((data.lineItems || data.LineItems || data.items || []) as Record<string, unknown>[]).map(li => this.normalizeLineItem(li)),
  ...
  private normalizeLineItem(data: Record<string, unknown>) {
    return {
      id: String(data.id || data.Id || ''),
  ...
  private normalizeClient(data: Record<string, unknown>): Client {
    return {
      id: String(data.id || data.Id || ''),
  ...
  private normalizeStatus(status: string): Invoice['status'] {
    const lower = status.toLowerCase();
    if (lower === 'paid' || lower === '1') return 'paid';
    if (lower === 'overdue') return 'overdue';
    return 'unpaid';
  }

  private normalizeInvoiceListResponse(data: Record<string, unknown>): { invoices: Invoice[]; total: number } {
    const invoices = ((data.invoices || data.Invoices || data.data || []) as Record<string, unknown>[]).map(inv => this.normalizeInvoice(inv));
    return {
      invoices,
  ...
  private normalizeClientListResponse(data: Record<string, unknown>): { clients: Client[]; total: number } {
    const clients = ((data.clients || data.Clients || data.data || []) as Record<string, unknown>[]).map(c => this.normalizeClient(c));
    return {
      clients,
  ...
  private mapCreateInvoiceRequest(payload: CreateInvoiceRequest) {
